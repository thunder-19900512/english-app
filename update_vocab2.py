...
raw_data = """1	rice	ご飯	食べ物	I'd like ◯◯.	12
...
def get_emoji(word):
    if word in old_emojis: return old_emojis[word]
...
for line in raw_data.strip().split('\n'):
    ...
    id_, english, japanese, category, keyPhrase, page = parts
    emoji = get_emoji(english)
    
    english_e = english.replace("'", "\\'")
    japanese_e = japanese.replace("'", "\\'")
    category_e = category.replace("'", "\\'")
    keyPhrase_e = keyPhrase.replace('"', '\\"')
    
    result.append(f"  {{ id: 'v{id_}', english: '{english_e}', japanese: '{japanese_e}', category: '{category_e}', page: {page}, emoji: '{emoji}', keyPhrase: \"{keyPhrase_e}\" }}")

joined_result = ",\n".join(result)
out_text = f"""export interface Vocabulary {{
  id: string;
  english: string;
  japanese: string;
  category: string;
...
with open("src/data/vocabulary.ts", "w") as f:
    f.write(out_text)
